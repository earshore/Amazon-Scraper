...
document.getElementById('scrapeBtn').addEventListener('click', async () => {
    ...
    try {
        ...
        // 查找当前站点应该使用的语言前缀
        const expectedLang = Object.keys(languageMap).find(key => host.includes(key));
        const currentExpectedPrefix = expectedLang ? languageMap[expectedLang] : null;
...
function renderPreview(prod) {
    ...
    if (prod.customer_reviews && prod.customer_reviews.length > 0) {
        ...
        reviewsHtml = '<p style="color: #565959; font-size: 13px; font-style: italic;">No reviews found for this product.</p>';
    }
...
function scrapeAmazonLogic() {
    try {
        ...
        const config = {
            ...
            // '[data-hook="review-title"] span:not(.a-letter-space)', // 标准版
            // '[data-hook="review-title"]',                         // 现代版备选
            // '.review-title-content span',                         // 移动端/新版布局
            // 'a[data-hook="review-title"]',                        // 链接形式标题
            // '.a-size-base.a-link-normal.review-title'             // 备用类名
],
